...
## 🔒 Cloudflare DNS 프록시(Proxy) 설정 가이드

DDoS 방어, SSL(HTTPS) 인증서 관리, 그리고 CDN 최적화를 위해 서비스 도메인(`myplating.kr`)을 Cloudflare에 연동하는 것을 권장합니다.

### 1. Cloudflare DNS 레코드 등록
Cloudflare 대시보드에 로그인한 후 도메인의 DNS 설정 메뉴에서 다음과 같이 레코드를 매핑합니다:

1. **프론트엔드 (Vercel 연동)**
   * **유형(Type):** `CNAME`
   * **이름(Name):** `@` (혹은 `www`)
   * **대상(Target):** `cname.vercel-dns.com`
   * **프록시 상태(Proxy Status):** `프록시됨 (Proxied)` (DDoS 보호 및 Cloudflare CDN 활성화)

2. **백엔드 (Render API 연동)**
   * **유형(Type):** `CNAME`
   * **이름(Name):** `api`
   * **대상(Target):** Render 대시보드 웹 서비스에 표시된 하위 도메인 (예: `plating-api.onrender.com`)
   * **프록시 상태(Proxy Status):** `프록시됨 (Proxied)`

### 2. SSL/TLS 암호화 모드 권장 설정
Cloudflare 프록시를 적용할 경우, 오리진 서버(Vercel, Render)와 Cloudflare 간의 통신 방식에 맞춰 SSL 모드를 설정해야 합니다.

* **권장 암호화 모드:** `Full (Strict)` 또는 `Full`
* **주의 (무한 리다이렉트 방지):** SSL 모드를 `Flexible`로 지정하면 브라우저-Cloudflare 구간은 HTTPS로 통신하지만 Cloudflare-Render 구간은 HTTP(80)로 통신하게 되어 Render 내부의 HTTPS 리다이렉션 미들웨어와 충돌해 `Too many redirects` 오류가 발생할 수 있습니다. 반드시 **`Full (Strict)`** 모드를 선택해 주십시오.
